...
\noindent Elle s'adresse aux étudiants en première année de médecine et aux étudiants en licence de biologie, mais pourra aussi intéresser des économistes qui voudraient utiliser des équations différentielles mais ne savant pas par où commencer avec un simple bagage en maths de lycée.\\
...
\noindent Cette fiche ne se substitue pas entièrement à celle du tutorat ou à un cours dispensé par votre prof, mais apporte des précisions et des explications qui permettent une meilleure compréhension des notions pas évidentes qui sont présentées dans le cours de Biostatistique ou de mathématiques pour la biologie.\\
...
Il est possible d'appeler l'ensemble des solutions d'une équation différentielle "\textbf{le flot}". C'est noté dans la fiche du tutorat sur les ED que j'ai lu donc j'imagine que c'est à connaître. \\
...
Le raisonnement par l'absurde permet de s'affranchir de ces contraintes en montrant que le théorème doit être vrai sans en expliciter la preuve en prouvant que si le résultat est faux, alors il y a une contradiction dans la théorie et donc comme on suppose que la théorie est cohérente, alors le théorème doit être vrai.\ Notez bien que ce type de raisonnement est nécesssaire en mathématique car il y a des résulats qu'on ne peut pas montrer sans.\\
...
Je ne ferrais évidement pas plus d'introduction à la logique formelle (tout bonnement car ça ne sert à rien ici), mais ça reste un domaine passionant pour les personnes qui se sont toujours posé la question "Mais pourquoi ce raisonement fonction, comment on montre que notre démonstration fonctionne ?". Je donne quand même la référence d'un excellent livre sur la logique formelle pour les plus curieux: \href{https://www.dunod.com/sciences-techniques/introduction-logique-theorie-demonstration-1}{Introduction à la logique (DUNOD)} (il dépasse très largement le cadre des cours que les lecteurs de cette fiche pourrait suivre mais il reste accessible à toute personne ayant un niveau terminale en mathématique.)
}
\end{tcolorbox}
...
\end{align}
Comme sans condition initiale, il n'y a pas d'unique solution, on dit que (2) est \textbf{une} solution d'un certain ($E_0$). Si on trouve C, on dira que c'est \textbf{la} solution d'un certain ($E_0$)

\subsubsection{Exemples}\label{ex1}

J'espère qu'à ce stade, vous n'êtes pas perdus. Si c'est le cas, c'est pas grave, on va réexpliquer tout ça avec des exemples:\\
...
La dérivée $f'(t)$ de $f(t)$ est $1$, donc en remplaçant, on a $2t - t = t$ donc $f(t) = t$ est solution particulière et les solutions générales de (E) s'écrivent sous la forme: $y = \lambda \sqrt{t} + t$. Vous pouvez donc constater que les solutions d'une EDO ne sont pas toujours sous la forme d'une exponentielle. 
\end{tcolorbox}
...
On considère l'ED suivante: \textbf{En construction}
\end{tcolorbox}
...
Ceci est une méthode très classique permettant de faciliter les opérations avec des quotients de racines (en particulier car il est très courant de rencontrer des racines quand on s'intéresse aux solutions d'un polynôme du second degré.) Elle s'applique à une grande variété de problèmes différents. En math c'est souvent pratique de pouvoir changer la forme de nos objets, en particulier, quand on a affaire à un produit, on peut toujours multiplier par 1. Donc, on pourrait écrire: 
$$
...
\noindent On constate que les solutions sont stables (pas de passage par une extermination d'une des populations par exemple) et périodiques (il y a d'abord moins de prédateur, mais à mesure que le nombrede proies augmente, il y a plus de prédateur jusqu'à un certain point où il n'y a plus suffisamment de proies et leur population décline et puis ça recommence). Ceci est illustré par le fait que le graphe boucle (à peu près.)\\ 
Ici, il faut faire attention au fait qu'on ne trace pas l'évolution des populations au cours du temps mais, en fonction de l'autre population. Par ailleurs, si le graphe ne boucle pas totalement, c'est à cause de la précision de la méthode numérique utilisée ainsi que les échelles de grandeur (ici 5000 ans).
...
\end{itemize}
C'est toujours un modèle non linéaire. Ce modèle, contrairement à Lotka-Volterra, ne met pas en évidence une périodicité de la quantité de proies et de prédateur, mais il prédit plutôt un équilibre entre les deux populations au bout d'un certain temps. 
\end{tcolorbox}
...
C'est peut-être un point important à souligner~: tout n'est pas modélisé par le même modèle. \textbf{Tous les modèles ont des hypothèses différentes}, il convient alors de déterminer quels sont les hypothèses qui collent le mieux avec nos données observées.\\

Enfin, on pourrait s'amuser à changer la loi de décès intrinsèques des prédateurs, modéliser un écosystème plus complexe en ajoutant des équations, pour par exemple, ajouter une espèce de plantes dont pourrait se nourrir les proies, etc. \\
Je vous laisserai chercher tout ça pour ceux que ça intéresse. Vous trouverez des informations utiles \href{https://math.univ-cotedazur.fr/~hoering/bio-s2-19/cours-lotka-volterra.pdf}{ici (cours de l'UniCA)} pour commencer et puis \href{https://www.eccorev.fr/IMG/pdf/Equations_LRoques.pdf#page=11}{sur ce cours} pour plus d'informations. 
\subsection{Pour aller plus loin sur les modèles en biologie et en économie*}
Vide


\begin{center}
\textbf{Fin. J'espère que cette fiche vous aura été utile.}\\
\textbf{Bon courage pour les études.}\\[5mm]
\textit{Si vous relevez des erreurs ou souhaitez formuler des remarques concernant cette fiche, n'hésitez pas à me contacter à l'adresse e-mail indiquée en première page de ce document.}
\end{center}
\newpage

\section{Références}
Uniquement mon imagination débordante hahaha. \\
Vous trouverez tous les liens nécessaires dans le corps du texte. Pour les parties "historiques", vous pouvez passer vos nuits à lire des pages Wikipédia dans l'espoir, peut-être, de gagner question pour un champion ou bien vous pouvez vous contenter des informations que j'ai données, sans que je puisse toutefois en garantir l'exactitude. 
